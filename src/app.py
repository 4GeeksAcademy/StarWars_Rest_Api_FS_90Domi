...
def push_planetfav(planet_id):
    ...
    if request.method == 'POST':
         planeta_fav = Favorites.query.filter_by(user_id=usuario_id, planet_id=planet_id).first()
         if not planeta_fav:
...
def post_peoplefav(people_id):
    ...
    people_fav= Favorites.query.filter_by(user_id = usuario_id,  people_id = people_id)
    if request.method == 'POST':
         if not people_fav:
